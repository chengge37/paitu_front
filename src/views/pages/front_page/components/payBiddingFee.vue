<template>
	<div class="wrap">
		<el-dialog :visible.sync="isShow" width="500px" class="dialog" @closed="closed" :modal-append-to-body="appendBody">
			<div class="text">
				支付方式：
			...
			<div class="text payMoney">
				支付金额：
				<span class="money red">￥{{payPrice}}</span>
			</div>
			<div class="code">
...
<script>
	...
	export default {
		...
		mounted() {
			console.log('payBiddingFee------------------', this.totalPrice);
		...
		methods: {
			...
			payOrder() {
				...
				console.log('payBiddingFee----------', params);
				this.util.payBiddingFee(params).then(res => {
			...
			getPayStatus() {
				...
				this.payInterval = setInterval(() => {
					this.util.getPayBiddingOrder(this.orderId).then(res => {
						//支付成功
						if(res.pay_status == 1) {
