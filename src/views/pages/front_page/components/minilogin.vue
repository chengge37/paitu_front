        ...
        <div class="login">
          ...
          <div class="login-bottom-box">
            ...
            <div class="three-box">
              <div
                class="box-item"
                v-for="(item,index) in threelogin"
                :key="index"
                @click="Threelogin(item.type)"
              >
...
<script>
/*                  mini登录组件
 *        接受参数是openlogin,接受一个布尔值
      接受参数是reg，接受一个布尔值，用于标示打开时是否为注册
...
export default {
  ...
  methods: {
    Threelogin(data) {
      console.log(data);
      this.getRequest(safe.getThreeCode, { type: data })
        .then(res => {
        ...
        if (this.is_reg) {
          this.$store.dispatch("CodeLogin", this.from_data).then(res => {
            if (res) {
              ...
              //   this.$parent.$parent.$parent.showDialog=true
              // }
              this.closed();
          ...
          this.postRequest(setting.forGot, this.from_data).then(res => {
            this.is_forgot = false;
            this.confirm();
          });
        } else {
