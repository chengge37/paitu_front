  ...
  <div class="wrap">
    <div class="header">
      ...
      <div class="search-wrap" v-if="false">
        <el-input
          placeholder="输入订单名称或订单号进行搜索"
          v-model="searchText"
          @keyup.enter.native="searchGetOrder(searchText)"
        ></el-input>
    ...
    <div class="content">
      ...
      <div class="label-group">
        ...
        <el-select
          class="label4"
          @change="selectStatu(checkedStatu)"
          v-model="checkedStatu"
          placeholder="订单状态"
        >
          <el-option
      ...
      <div class="order-item" v-for="(item,index) in orderList" :key="index">
        ...
        <div class="item-table">
          <div class="col1">
            <!--<img :src="$qiniuHost+item.service_pic.split(',')[0]" alt="出错">-->
            <div style="width:88px;height:88px;margin-right:10px;">
            	<el-image  style="width:100%;height:100%;" fit="contain" :src="util.picExplode(item.service_pic)"
				class="cursor" @click="toServiceDetail(item.service_id)"/>
			</div>
            <div class="right-dec">
              <span class="cursor word-hover" @click="toServiceDetail(item.service_id)">{{item.service_title}}</span>
              <div>
                <span>具体规格：</span>
                <span class='text-wrap'>{{item.norms_name}}&nbsp;{{item.norms_content}}</span>
              </div>
            </div>
          </div>
          <div class="col2">
          ...
          <div class="col4">
            ...
            <span class="cursor word-hover" @click="util.toBuyOrder(1,item.id)">订单详情</span>
          </div>
          <div class="col5">
            <span class="cursor word-hover" v-if="item.pay_status==1" @click="util.toBuyOrder(1,item.id)">立即支付</span>
    ...
    <!--退款窗口-->
    <user-refund-pay :show.sync="showRefund" :id="selectRefundId" type='8' @refundSuccess="refundSuccess" @closed="closeRefund"></user-refund-pay>
    
    <!--查看退款状态窗口-->
    <refund-status :openRefund.sync="showRefundStataus" @closed="closeRefundStatus" type='0' :orderId='selectRefundId' orderType='8'></refund-status>
...
import { service } from "@config/api.js"
  export default {
    ...
    created(){
      this.getOrders({page:1,page_size:3})
    },
    filters:{
      // 后台返回订单状态码转状态
      statusFormat(status) {
    ...
    toServiceDetail(id){
      ...
      selectStatu(val){
        ...
        this.getOrders({page:1,page_size:3,pay_status:val})
      },
      getOrders(params){
        console.log(params,'999')
        this.getRequest(service.getOwnOrder,params).then(res=>{
          console.log(res,'服务订单')
      ...
      changeTab(i){
        ...
        this.getOrders({page:1,page_size:3,pay_status:this.tabs[i].value})
        this.checkedStatu=this.tabs[i].value
      },
      searchGetOrder(){
        this.getOrders({page:1,page_size:3,search:this.searchText})
      },
      changePage(i){
        this.getOrders({page:i,page_size:3})
      },
      cancelOrder(id){
         this.$confirm("确定要取消订单吗？", "提示", {
          ...
          this.postRequest(model.userDelModelOrder,{id}).then(res=>{
            this.getOrders({page:1,page_size:3})
          }).catch(err=>{
            console.error(err)
          })
