				...
				<div class="info">
					...
					<div class="more">
						<div>
							...
							<p class="tip" v-if="optionIndex==1 && parseInt(studioItem.hour_num)>1">({{studioItem.hour_num}}小时起租)</p>
						</div>
...
//			},
		...
		methods: {
			...
			//判断选择的小时是否等于或者超过最小起拍小时
			checkHour(){
				//按小时租赁
				if(this.optionIndex==1){
					if(this.hours<parseInt(this.studioItem.hour_num)){
			...
			getDetailById(id) {
				...
				this.getRequest(frontStudio.getDetailById, params)
					.then(res => {
						...
						//获取相关收藏情况
						this.getCollect();
						console.warn("condition----", this.condition);
						//获取店铺信息
						this.getShopInfo(this.studioItem.uid);
