<template>
	<div class="mainContent">
		...
		<div class="main_Info">
			...
			<div class="sort">
				<ul>
					<li class="cursor" v-for="(item,index) in sortList" :key="index" :class="{'active':sortIndex==index}" @click="changeOrder(index)">
						<span>{{item}}</span>
					</li>
				</ul>
			...
			<div class="goodList">
				<ul>
					<li v-for="(item,index) in list" :key="index" class="card-hover">
						<el-image style="width:280px;height:158px;" fit="contain" :src="item.pic"  :onerror="util.errorPic" @click="toStudioDetail(item.id)" class="cursor"/>
						<p class="word-hover cursor" @click="toStudioDetail(item.id)">{{item.name}}</p>
						<div class="detail">
							<div class="priceInfo">
								<span class="price">￥{{item.priceWorkDayWorkTimePerHour}}</span>
	...
	export default {
		data() {
			return {
				...
				sortList:['综合排序','价格','销量'],
				list:[],
				pickerOptionsStart:this.util.checkDateStart(this,'dateStart','dateEnd'),
		...
		watch:{
			...
			square: function(to, from) {
				...
				console.log("to---", to);
				if(to > 0) {
					this.studioSizeList.forEach((item, index) => {
						if(item.id == to) {
							value = item.name;
						}
					});
			...
			studioType: function(to, from) {
				let value = "";
				if(to > 0) {
					this.studioList.forEach((item, index) => {
						if(item.id == to) {
							value = item.name;
						}
					});
		...
		methods: {
			...
			getStudioList() {
				...
				this.util.getStudioList(params).then(res=>{
						if(this.clearAll) {
							this.clearAll = false;
						}
						...
						if(res.rows.length > 0) {
							res.rows.forEach((item, index) => {
								if(item.pic.indexOf(",") > 0) {
									item.pic = this.$qiniuHost + item.pic.split(",")[0];
								} else {
