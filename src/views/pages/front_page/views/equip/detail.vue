<template>
	<div class="main_content">
		<div class="head">
			<div class="bread">
				<el-breadcrumb separator-class="el-icon-arrow-right">
					...
					<el-breadcrumb-item v-if="equipInfo">{{equipInfo.custom_desc||equipStatic.name}}</el-breadcrumb-item>
				</el-breadcrumb>
			</div>
		...
		<div class="body">
			...
			<!--套餐模块-->
			<div class="pack" v-if="packList.length>0">
				<p>爆款套餐</p>
				<ul class="packList">
					<li v-for="(item,index) in packList" :key="index">
						<div class="picList">
							<div @click="toDetai(item2.type,item2.id)" v-for="(item2,index2) in item.pic" :key="index2" class="cursor">
								<el-image  style="width:130px; height: 130px" :src="util.picExplode(item2.pic)" fit="contain">
								</el-image>
								<p v-if="index2==0">+</p>
							</div>
						</div>
						<div class="price">
							<p>
								<span>套餐价格：</span>
								<span>{{item.price}}/{{item.only_rent_day==0?'小时':'天'}}</span>
							</p>
							<el-button type="primary" class="color-area" @click="toPackDetail(item.id)">套餐详情</el-button>
						</div>
					</li>
				</ul>
			</div>
...
//			},
      ...
      ...mapGetters(["user_data"])
		...
		methods: {
			...
			//跳转到套餐详情页面
			toPackDetail(id){
				this.util.toPackDetail(id);
			},
			
			//根据设备id获取相关套餐
			getPack(id){
				console.error('getPack-----------',id);
				let params={
					type:'2', //1影棚 2设备
					relation_id:id,
				}
				this.util.getPackByProduct(params).then(res=>{
					console.warn('getPackByProduct-----',res);
					res.forEach((item,index)=>{
						this.packAjaxDetail(item.pack_id);
					})
				})
			},
			...
			packAjaxDetail(id){
				this.util.packAjaxDetail(id).then(res=>{
					console.error('packAjaxDetail-----',res);
					let obj={};
					obj.id=res.id;
					obj.only_rent_day=res.only_rent_day;
					obj.price=res.price_arr[0].price;
					obj.pic=[];
					let rel=res.rel;
					//影棚套餐
					if(res.ptm_type==1){
						let o={
							id:rel.studio_id.id,
							name:rel.studio_id.name,
							pic:rel.studio_id.pic,
							type:1,
						}
						obj.pic.push(o);
					}
					//设备套餐
					rel.equip_ids.forEach((item,index)=>{
						let j={
							id:item.equip_id,
							name:item.custom_desc||item.name,
							pic:item.equip_pic,
							type:2
						}
						obj.pic.push(j);
					...
					let tempObj;
					if(obj.pic.length>0){
						for(let i=0;i<obj.pic.length;i++){
							if(obj.pic[i].id==this.equipId){
								tempObj=obj.pic[i];
								obj.pic.splice(i,1);
								break;
							}
					...
					obj.pic.unshift(tempObj);
					this.packList.push(obj);
					console.error('packList------------',this.packList);
				})
			},
