	...
	<div class="newServe">
		<div class="act-order-container">
			...
			<el-table :data="orderList" v-loading="listLoading" element-loading-text="Loading" fit class="table-control">
				...
				<el-table-column label="模特标题" align="center">
					<template slot-scope="{row}">
						<span>{{row.title?row.title:row.real_name}}</span>
					</template>
				</el-table-column>
			...
			editOrder(type,item){
				...
				if(type==0){
					...
					this.util.confirm(this,'确定接单吗？').then(res=>{
						console.log('params----',params);
						this.util.editModelOrder(params).then(res=>{
...
//				let params = this.validParams();
			...
			//跳转到订单详情
			gotoDetail(data) {
				let path="/serve/order/modelOrderDetail"
				this.$router.push({
					path: path,
					query:{
						id:data.id
