	...
	<div class="newServe">
		<div class="act-order-container">
			...
			<el-table :data="orderList" v-loading="listLoading" element-loading-text="Loading" fit class="table-control">
				...
				<el-table-column label="产品标题" prop="service_title" align="center"></el-table-column>
				<el-table-column label="图片" align="center">
					<template slot-scope="{row}">
						<el-image style="width:60px; height: 60px" :src="util.picExplode(row.service_pic)" fit="contain"></el-image>
					</template>
				</el-table-column>
				<el-table-column label="订单编号" prop="show_id" align="center">
				</el-table-column>
				<el-table-column label="订单单价(元)">
					<template slot-scope="{row}">
						<span>{{row.norms_price}}</span>
					</template>
				</el-table-column>
				<el-table-column label="数量" prop="count" align="center">
				</el-table-column>
				<el-table-column label="订单总价(元)">
					<template slot-scope="{row}">
		...
		<!--操作退款窗口模块-->
    	<refund :openRefund="openRefund" :orderId="refundId" orderType="8" @closed="closeRefund"></refund>
		...
		methods: {
			...
			editOrder(type,item){
				...
				if(type==0){
					...
					this.util.confirm(this,'确定接单吗？').then(res=>{
						console.log('params----',params);
						this.util.acceptServiceOrder(params).then(res=>{
					...
					this.util.prompt(this,'请输入拒绝原因：','拒绝订单').then(({value})=>{
						params.reject_msg=value;  //拒绝原因
						console.log('params----',params);
						this.util.rejectServiceOrder(params).then(res=>{
			...
			editPrice(item){
				...
				this.util.prompt(this,'请输入不少于1的正整数：','修改订单价格').then(({value})=>{
					params.price=value; 
					console.log('params----',params);
					this.util.editServiceOrderPrice(params).then(res=>{
			...
			getOrderList() {
				...
				this.util.serviceOrderList(params).then(res=>{
					this.listLoading = false;
					this.totalCount=res.total_count;
					this.orderList=res.rows;
					//给列表添加一个价格字段
					this.orderList.forEach((item,index)=>{
						item.allPrice=(parseInt(item.norms_price)*parseInt(item.count)).toFixed(2);
					})
			...
			//跳转到订单详情
			gotoDetail(data) {
				let path="/serve/order/serviceOrderDetail"
				this.$router.push({
					path: path,
					query:{
						id:data.id
