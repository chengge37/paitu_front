            ...
            el.addEventListener('click',function(e){
                ...
                if(e.target.className.indexOf('currentDate') > -1){
					...
					if(dateList.length>0){
						if(dateList.indexOf(selectedDate)>=0){
                    ...
                    opt.clickCb && opt.clickCb(dateList);
                    render();
                }
            },false)
        }
            ...
            for(var i = 0; i < total; i++){
                if(i<startWeek){
					...
					for(var j=0;j<dateList.length;j++){
						dateList[j]==nowDate && (addClass = 'selected-style');
					}
//              	console.log('disableDate----',disableDate);
                	if(disableDate.length>0){
                		disableDate.forEach((item,index)=>{
                			item==nowDate && (addClass = 'disableDate');
                		})
                	}
